...
EDGE_SAMBA_UPLOAD_WAIT_PERIOD="${EDGE_SAMBA_UPLOAD_WAIT_PERIOD:-${INGEST_WAIT_PERIOD:-60}}"
EDGE_SAMBA_UPLOAD_VISIT="${EDGE_SAMBA_UPLOAD_VISIT:-samba_upload}"
EDGE_SAMBA_UPLOAD_SCAN="${EDGE_SAMBA_UPLOAD_SCAN:-1.SambaUpload}"
EDGE_SAMBA_UPLOAD_RESOURCE="${EDGE_SAMBA_UPLOAD_RESOURCE:-FILES}"
EDGE_SAMBA_UPLOAD_MODALITY="${EDGE_SAMBA_UPLOAD_MODALITY:-MR}"
# An explicitly empty raw allow-list admits all projects; only unset inherits DICOM policy.
EDGE_SAMBA_UPLOAD_ALLOWED_PROJECTS="${EDGE_SAMBA_UPLOAD_ALLOWED_PROJECTS-${EDGE_AUTO_IMPORT_ALLOWED_PROJECTS}}"
...
if [ "${AIS_EDGE_NO_SSH:-}" = "1" ] || [ "${EDGE_JOIN_MODE:-ssh}" = "manual" ]; then
    ...
    if [ "${EDGE_SAMBA_UPLOAD_ENABLED}" = "1" ]; then
        echo "  ${EDGE_SAMBA_UPLOAD_HOST_PATH}"
        echo "  ${EDGE_SAMBA_UPLOAD_ARCHIVE_HOST_PATH}"
    fi
else
    ssh ${SSH_KEY_OPT} "${EDGE_SSH}" \
        "sudo mkdir -p '${EDGE_DATA_HOST_PATH}/staging' && sudo chmod 777 '${EDGE_DATA_HOST_PATH}/staging' && if [ '${EDGE_SAMBA_UPLOAD_ENABLED}' = '1' ]; then sudo mkdir -p '${EDGE_SAMBA_UPLOAD_HOST_PATH}' '${EDGE_SAMBA_UPLOAD_ARCHIVE_HOST_PATH}' && sudo chmod 777 '${EDGE_SAMBA_UPLOAD_HOST_PATH}' '${EDGE_SAMBA_UPLOAD_ARCHIVE_HOST_PATH}'; fi"
...
render_with_topology "${REPO_DIR}/manifests/02-edge/xnat-ingest.yaml.tpl" \
    ...
    EDGE_AUTO_IMPORT_ALLOWED_PROJECTS "$EDGE_AUTO_IMPORT_ALLOWED_PROJECTS" \
    EDGE_SAMBA_UPLOAD_ENABLED "$EDGE_SAMBA_UPLOAD_ENABLED" \
    EDGE_SAMBA_UPLOAD_HOST_PATH "$EDGE_SAMBA_UPLOAD_HOST_PATH" \
    EDGE_SAMBA_UPLOAD_ARCHIVE_HOST_PATH "$EDGE_SAMBA_UPLOAD_ARCHIVE_HOST_PATH" \
    EDGE_SAMBA_UPLOAD_WAIT_PERIOD "$EDGE_SAMBA_UPLOAD_WAIT_PERIOD" \
    EDGE_SAMBA_UPLOAD_VISIT "$EDGE_SAMBA_UPLOAD_VISIT" \
    EDGE_SAMBA_UPLOAD_SCAN "$EDGE_SAMBA_UPLOAD_SCAN" \
    EDGE_SAMBA_UPLOAD_RESOURCE "$EDGE_SAMBA_UPLOAD_RESOURCE" \
    EDGE_SAMBA_UPLOAD_MODALITY "$EDGE_SAMBA_UPLOAD_MODALITY" \
    EDGE_SAMBA_UPLOAD_ALLOWED_PROJECTS "$EDGE_SAMBA_UPLOAD_ALLOWED_PROJECTS" \
