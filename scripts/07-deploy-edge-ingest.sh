...
if [ "${AIS_EDGE_NO_SSH:-}" = "1" ] || [ "${EDGE_JOIN_MODE:-ssh}" = "manual" ]; then
    ...
    ssh ${SSH_KEY_OPT} "${EDGE_SSH}" \
        "sudo mkdir -p '${EDGE_DATA_HOST_PATH}/staging' && sudo chmod 777 '${EDGE_DATA_HOST_PATH}/staging' && if [ '${EDGE_SAMBA_UPLOAD_ENABLED}' = '1' ] && [ ! -d '${EDGE_SAMBA_UPLOAD_HOST_PATH}' ]; then sudo mkdir -p '${EDGE_SAMBA_UPLOAD_HOST_PATH}' && sudo chmod 777 '${EDGE_SAMBA_UPLOAD_HOST_PATH}'; fi"
